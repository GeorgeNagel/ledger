...
class TestGetAccountBalance(TestCase):
    ...
    def test_journal_entries_no_statement(self):
        ...
        JournalEntryDetail.objects.create(
            account=account, journal_entry=journal_entry, amount=123, normal=JournalEntryDetail.Normals.DEBIT
        )
        balance = account.get_balance()
        self.assertEqual(balance, 123)
    ...
    def test_journal_entries_since_statement(self):
        ...
        journal_entry = JournalEntry.objects.create(date_effective=timezone.now())
        JournalEntryDetail.objects.create(
            account=account, journal_entry=journal_entry, amount=50, normal=JournalEntryDetail.Normals.DEBIT
        )
        balance = account.get_balance()
    ...
    def test_multiple_statements(self):
        account = Account.objects.create(normal=Account.Normals.DEBIT)
        Statement.objects.create(
            account=account, date_close=datetime(2000, 1, 1), balance=100
        )
        first_journal_entry = JournalEntry.objects.create(date_effective=datetime(200, 1, 2))
        JournalEntryDetail.objects.create(
            account=account, journal_entry=first_journal_entry, amount=50, normal=JournalEntryDetail.Normals.DEBIT
        )

        second_journal_entry = JournalEntry.objects.create(date_effective=datetime(2000, 1, 3))
        JournalEntryDetail.objects.create(
            account=account, journal_entry=second_journal_entry, amount=60, normal=JournalEntryDetail.Normals.DEBIT
        )

        Statement.objects.create(
            account=account, date_close=datetime(2000, 2, 1), balance=200
        )
        third_journal_entry = JournalEntry.objects.create(date_effective=datetime(2000, 2, 2))
        JournalEntryDetail.objects.create(
            account=account, journal_entry=third_journal_entry, amount=70, normal=JournalEntryDetail.Normals.DEBIT
        )
        balance = account.get_balance()
    ...
    def test_credit_journal_entries_increase_credit_normal_account_balance(self):
        ...
        journal_entry = JournalEntry.objects.create(date_effective=timezone.now())
        JournalEntryDetail.objects.create(
            account=account, journal_entry=journal_entry, amount=50, normal=JournalEntryDetail.Normals.CREDIT
        )
        balance = account.get_balance()
        self.assertEqual(balance, 50)
